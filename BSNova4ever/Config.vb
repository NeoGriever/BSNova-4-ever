...
Public Partial Class Config
	...
	Sub Button4Click(sender As Object, e As EventArgs)
		GlobalConfig.GlobalConfig.DBInsert("downloader.path",textBox1.Text)
		GlobalConfig.GlobalConfig.DBInsert("pattern.movie",textBox2.Text)
		GlobalConfig.GlobalConfig.DBInsert("pattern.episode",textBox3.Text)
	...
	Sub ConfigLoad(sender As Object, e As EventArgs)
		Dim TargetPath As String = System.Environment.GetFolderPath(System.Environment.SpecialFolder.CommonVideos)
		If TargetPath.Substring(TargetPath.Length - 1,1) <> "\" Then
			TargetPath &= "\"
		End If
		TargetPath = GlobalConfig.GlobalConfig.DBSelect("downloader.path",TargetPath,True)
		If TargetPath.Substring(TargetPath.Length - 1,1) <> "\" Then
			TargetPath &= "\"
		End If
		textBox1.Text = TargetPath
		Dim patt_movie As String = GlobalConfig.GlobalConfig.DBSelect("pattern.movie","%serie%\%name%.mp4",True)
		Dim patt_episo As String = GlobalConfig.GlobalConfig.DBSelect("pattern.episode","%serie%\S%seasonNN%E%episodeNN% - %name%.mp4",True)
